...
parser.add_argument(
    ...
    '--input_dir',
    type=str,
    default='/home/stu3/datasets/cvpr_medsam_val/',
    # required=True,
...
parser.add_argument(
    ...
    #default="workdir/test0415_b8_allm/medsam_lite_best.pth",
...
def MedSAM_infer_npz_2D(img_npz_file):
    ...
    if save_overlay:
        overlay_image = img_3c.copy()
        for i, box in enumerate(boxes):
            color = tuple([int(x * 255) for x in np.random.rand(3)])
            top_left, bottom_right = box[:2], box[2:]
            cv2.rectangle(overlay_image, top_left, bottom_right, color, 2)
            mask = (segs == i + 1).astype(np.uint8) * 255
            overlay_image[mask > 0] = overlay_image[mask > 0] * 0.5 + np.array(color) * 0.5
        cv2.imwrite(join(png_save_dir, npz_name.split(".")[0] + '.png'), overlay_image)        
...
def MedSAM_infer_npz_3D(img_npz_file):
    ...
    if save_overlay:
        ...
        img = img_3D[idx].astype(np.uint8)  
        ...
        for i, box3D in enumerate(boxes_3D, start=1):
            if np.sum(segs[idx] == i) > 0:
                ...
                mask = (segs[idx] == i).astype(np.uint8) * 255
                img_color[mask > 0] = img_color[mask > 0] * 0.5 + np.array(color) * 0.5  
    
        output_file = join(png_save_dir, npz_name.split(".")[0] + '.png')
